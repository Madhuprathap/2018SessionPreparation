...
public class BalanceLog {
	public double balancePoint(Log log) {
		double balaneWeight = log.weightUpto(log.length())/2;
		double i = log.length()/2;
		while(true) {
			double temp = log.weightUpto(i);
			//base condition
			if (Math.abs(temp-balaneWeight) <= 0.001) {
				break;
			}
			if (temp > balaneWeight) {
				i = i - 0.001;
			} else {
				i = i + 0.001;
			}
